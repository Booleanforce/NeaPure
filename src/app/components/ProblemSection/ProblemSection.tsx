...
import { useGetProblemStatsQuery } from "@/features/problem-stats/api/problemStatsApi";
...
// Fallback in case the API hasn't returned yet or fails
const fallbackStats = [
  {
    icon: "droplet",
    value: "80%",
...
export default function ProblemSection() {
  const { data, isLoading, isError } = useGetProblemStatsQuery();
  const stats = !isLoading && !isError && data ? data : fallbackStats;
          ...
          >
            {isLoading
              ? Array.from({ length: 4 }).map((_, i) => (
                  <div
                    key={i}
                    className="min-h-[170px] animate-pulse rounded-xl bg-white/60 xl:h-[185px] xl:min-h-0 2xl:h-[190px]"
                  />
                ))
